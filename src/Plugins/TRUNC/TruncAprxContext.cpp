...
bool TruncContext::TruncAprxContext::readReport(::std::string reportPath)
{
  ...
  log::BellerophonLogger::verbose("Reading a TRUNC report:\n");
  while (report.read_row(OpId, line, Op1, OpTy, Op2, OpRet))
  {
    ...
    // Build an instance of TruncAprxTechnique
    TruncAprxTechnique  tac(this->LocStartId, OpId, Op1, Op2, OpTy);
    this->LocStartId++;
    
    // Build an instance of AprxLocation
    core::AprxLocation l(::std::make_shared<TruncAprxTechnique>(tac));
